...
public class GameActionExecuteMultiple : GameAction<GameActionExecuteMultiple.Settings>
{
    ...
    {
        var buffer = input.Accessor.GetBuffer<ActionsBufferElement>(input.Context.Action);
        foreach (ActionsBufferElement gameAction in buffer)
        {
            CommonWrites.RequestExecuteGameAction(input.Accessor, input.ActionInstigator, gameAction.ActionEntity, input.Context.Targets, input.Parameters);
        }
