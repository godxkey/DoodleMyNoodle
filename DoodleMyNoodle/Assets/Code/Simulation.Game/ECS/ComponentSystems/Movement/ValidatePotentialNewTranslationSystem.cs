            ...
            ref PotentialNewTranslation newTranslation) =>
        {
            ...
            if (!tileDestinationPos.Equals(currentTilePos))
            {
                if (!CommonReads.DoesTileRespectFilters(Accessor, CommonReads.GetTile(Accessor, tileDestinationPos), TileFilterFlags.Inoccupied | TileFilterFlags.Navigable))
                {
                    ...
                    //velocity = new Velocity() { Value = new fix3(0,0,0) };
                }
            }
...
public partial class CommonReads
{
    ...
    {
        List<Entity> tileAddons = CommonReads.GetTileAddons(accessor, tile);
        ...
        if (tileAddons.Count > 0)
        {
            foreach (Entity addon in tileAddons)
            {
                ...
                {
                    if (accessor.HasComponent<NonNavigable>(addon))
                    {
                        return false;
                    }
                }
                ...
                {
                    if (accessor.HasComponent<Occupied>(addon))
                    {
                        return false;
                    }
                }
