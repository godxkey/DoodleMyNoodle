...
public class GlobalVFXHandler : GamePresentationSystem<GlobalVFXHandler>
{
    ...
    public override void PresentationUpdate()
    {
        foreach (var gameActionEvent in PresentationEvents.GameActionEvents.SinceLastPresUpdate)
        {
            ...
            if (!SimWorld.HasComponent<PeriodicAction>(gameActionEvent.GameActionContext.ActionInstigator) ||
                (SimWorld.TryGetComponent(gameActionEvent.GameActionContext.ActionInstigator, out PeriodicAction autoAttack) && autoAttack.Value != gameActionEvent.GameActionContext.Action))
            {
                SimWorld.TryGetComponent(gameActionEvent.GameActionContext.ActionInstigator, out SimAssetId instigatorAssetId);
