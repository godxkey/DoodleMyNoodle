...
namespace NestedTODO
{
    ...
    public class NestedList : ScriptableObject
    {
        ...
        public class BurndownData
        {
            ...
            public List<float> dateValues = new List<float>();
        }

        [SerializeField, HideInInspector]
        public BurndownData burndownData;
        ...
        void OnEnable()
        {
            ...
            if (listConfig == null)
                listConfig = new ListConfiguration(SerializedListObject);
            else
            {
        ...
        public void MarkBranch(NestedNode node, bool state)
        {
            ...
            SerializedNode(node.NodeID).FindPropertyRelative("completed").boolValue = state;
        ...
        public void TellParentToCheckChilds(int id)
        {
            ...
            SerializedNode(parent.NodeID).FindPropertyRelative("completed").boolValue = tmpBool;
            SaveSerializedProperties();
        ...
        //functions to handle the burndown chart data
        #region burndown functions
        ...
        {
            var burndownDates = SerializedListObject.FindProperty("burndownData.date");
            var burndownValues = SerializedListObject.FindProperty("burndownData.dateValues");

            if (burndownData.date.Count == 0)
            {
                burndownDates.arraySize++;
                burndownDates.GetArrayElementAtIndex(0).stringValue = System.DateTime.Now.Date.ToShortDateString();

                burndownValues.arraySize++;
                burndownValues.GetArrayElementAtIndex(0).floatValue = SumNodePoints();

                SaveSerializedProperties();
            }
            else
            {
                while (burndownData.date.Count > 1 && burndownData.dateValues[0] == 0)
                {
                    burndownDates.DeleteArrayElementAtIndex(0);
                    burndownValues.DeleteArrayElementAtIndex(0);
                    SaveSerializedProperties();
                }
            ...
            var lastDate = System.DateTime.Parse(burndownData.date[burndownData.date.Count - 1]);
            int result = System.DateTime.Compare(currentDate, lastDate);

            if (result < 0)
            {
                Debug.Log("Somehow, the last known date is somewhere in the future");
            }
            //the last known date is today
            if (result == 0)
            {
                if (burndownData.date.Contains(currentDate.ToShortDateString()))
                {
                    int i = burndownData.date.IndexOf(currentDate.ToShortDateString());
                    burndownValues.GetArrayElementAtIndex(i).floatValue = SumNodePoints();
                }
                else
                {

                    burndownDates.arraySize++;
                    burndownDates.GetArrayElementAtIndex(burndownData.date.Count - 1).stringValue = currentDate.ToShortDateString();

                    burndownValues.arraySize++;
                    burndownValues.GetArrayElementAtIndex(burndownData.dateValues.Count - 1).floatValue = SumNodePoints();
                }

                SaveSerializedProperties();
            }
            //last date is older than today
            if (result > 0)
            {
                float lastValue = burndownData.dateValues[burndownData.date.Count - 1];
                int i = burndownData.date.Count;
                while (System.DateTime.Compare(currentDate, lastDate) > 0)
                {
                    lastDate = lastDate.AddDays(1);

                    burndownDates.arraySize++;
                    burndownDates.GetArrayElementAtIndex(i).stringValue = lastDate.ToShortDateString();

                    burndownValues.arraySize++;
                    burndownValues.GetArrayElementAtIndex(i).floatValue = lastValue;
    ...
    public class NestedNode
    {
        ...
        public Object LinkedFile { get { return this.linkedFile; } set { this.linkedFile = value; } }
