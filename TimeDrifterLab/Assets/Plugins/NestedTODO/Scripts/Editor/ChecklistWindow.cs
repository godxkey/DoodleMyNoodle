...
namespace NestedTODO
{
    public class ChecklistWindow : EditorWindow
    {
        ...
        void DrawListElement(NestedNode node, bool drawActive)
        {
            ...
            EditorGUI.BeginChangeCheck();
            serializedNode.FindPropertyRelative("completed").boolValue = EditorGUILayout.Toggle(node.Completed, GUILayout.Width(15));
            if (EditorGUI.EndChangeCheck())
            {
                changesMade = true;
            ...
            if (node.Completed && node != activeNode)
                tmpColor = Config.getColor(4);
            else if (node == activeNode)
                tmpColor = Config.getColor(5);
            else
                tmpColor = Config.getColor(3);
            ...
            if (!Config.HideCategories)
            {
                ...
                EditorGUI.BeginChangeCheck();
                tmpProperty.intValue = EditorGUILayout.Popup(tmpProperty.intValue, categories, GUILayout.MaxWidth(50));
                if (EditorGUI.EndChangeCheck())
                {
                    changesMade = true;
                    if (Config.AutoCategory)
            ...
            if (!Config.HideTags)
            {
                ...
                EditorGUI.BeginChangeCheck();
                tmpProperty.intValue = EditorGUILayout.Popup(tmpProperty.intValue, tags, GUILayout.MaxWidth(50));
                if (EditorGUI.EndChangeCheck())
                {
                    changesMade = true;
                    if (Config.AutoTag)
